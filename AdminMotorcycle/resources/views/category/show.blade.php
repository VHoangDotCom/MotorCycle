                                    ...
                                    @foreach ($categories as $category)
                                        @if ($category->parent_id == null)
                                            <li style="cursor: pointer" class="cate-pro">
                                                {{ $category->categoryName }}<span>({{ $category->children->count() }})</span><i
                                                    class="fas fa-angle-down ml-2"></i>
                                                <ul class="cate-s">
                                                    @foreach ($category->children as $c)
                                                        <li><a
                                                                href="{{ route('category.show', $c->cate_id) }}">-{{ $c->categoryName }}<span>({{ $c->products->count() }})</span></a>
                                                        </li>
                                                    @endforeach
                                                </ul>
                                            </li>
                                    ...
                                    <div class="sort-box-holder">
                                        <div class="btn-group pull-right search-right-box">
                                            <form action="{{ route('category.show', $cate->cate_id) }}" class="category-search-frm">
                                                <input type="text" name="filter" placeholder="Tìm trong {{ $cate->categoryName }} ...">
                                ...
                                <div class="col-md-3 pro-hov">
                                    ...
                                    <div class="product-bottom text-center">
                                        ...
                                        <p class="title"><a style="color: rgb(73, 72, 72)"
                                                            href="{{ route('product.show', $pro->pro_id) }}">{{ $pro->productName }}</a>
                                        </p>
