                                ...
                                <ul class="cate-pro-item">
                                    @foreach ($cate as $category)
                                        @if ($category->parent_id == null)
                                            <li><a href="{{ route('category.show', $category->cate_id) }}">{{ $category->categoryName }}<span>({{ $category->children->count() }})</span></a>
                                                <ul>
                                                    @foreach ($category->children as $c)
                                                        <li><a href="{{ route('category.show', $c->cate_id) }}">-{{ $c->categoryName }}<span>({{ $c->products->count() }})</span></a></li>
                                                    @endforeach
                                                </ul>
                                            </li>
                                        @endif
                            ...
                            @foreach($resultFind as $pro)
                                <div class="col-md-3 pro-hov">
                                    ...
                                    <div class="product-bottom text-center">
                                        ...
                                        <p class="title"> {{ $pro->productName }}</p>
